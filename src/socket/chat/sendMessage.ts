...
export async function handleSendMessage(
  ...
  // Use message's actual createdAt as the lastSeen timestamp (server time, always reliable)
  const messageCreatedAt: Date = saveMessage.createdAt;

  // Check if receiver is active in this conversation room
  ...
  if (room && room.size > 1 && receiverId) {
    console.log(`Checking active sockets in room ${conversationIdString} for receiver ${receiverId} room size: ${room.size}`);
  ...
  // Build lastSeen updates — store Date, not ObjectId
  const lastSeenUpdates: Record<string, Date> = {
    [senderId]: messageCreatedAt, // sender always seen their own message
  };
  if (receiverIsSeen && receiverId) {
    lastSeenUpdates[receiverId] = messageCreatedAt; // receiver is active, auto-seen
  }
  ...
  // Set each participant's lastSeen date
  Object.entries(lastSeenUpdates).forEach(([userId, seenAt]) => {
    updateFields[`lastSeen.${userId}`] = seenAt;
  });
  ...
  // Notify sender that receiver has auto-seen
  if (receiverIsSeen && receiverId) {
    io.to(senderId).emit(SOCKET_EVENTS.MESSAGES_SEEN, {
      conversationId: conversationIdString,
      seenBy: receiverId,
      lastSeenAt: messageCreatedAt.toISOString(),
    });
  }

  // Unread count per participant using createdAt comparison
  for (const participantId of conversation.participants) {
    const participantIdString = participantId.toString();

    // Use the updated lastSeen date if we just set it, otherwise use stored value
    const participantLastSeenDate: Date | undefined =
      lastSeenUpdates[participantIdString] ??
      (conversation.lastSeen as any)?.[participantIdString];

    const unreadCount = participantLastSeenDate
      ? await Message.countDocuments({
          conversationId: conversationObjectId,
          senderId: { $ne: new Types.ObjectId(participantIdString) },
          createdAt: { $gt: participantLastSeenDate }, // Date comparison
        })
      : await Message.countDocuments({
          conversationId: conversationObjectId,
          senderId: { $ne: new Types.ObjectId(participantIdString) },
    ...
    io.to(participantIdString).emit(SOCKET_EVENTS.CONVERSATION_UPDATED, {
     ...
     const totalUnread = await getUnreadMessageCount(participantIdString);
    io.to(participantIdString).emit(SOCKET_EVENTS.UNREAD_MESSAGE_COUNT, {
