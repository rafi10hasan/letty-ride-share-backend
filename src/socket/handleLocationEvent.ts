...
const handleLocationEvents = async (
    ...
    socket.on(SOCKET_EVENTS.DRIVER_LOCATION_UPDATE, async (data: {
        ...
        }).populate<{ passenger: IPopulatedPassenger }>("passenger", "user fcmToken").lean();
        ...
        if (timeSinceLastUpdate >= 60 * 1000) {
            ...
            if (ride) {
                ...
                const { etaSeconds, distanceMeters } = await getETAFromGoogleMaps(
                    data.coordinates,
                    [ride.dropOffLocation.coordinates[1], ride.dropOffLocation.coordinates[0]]
                );

                const estimatedArrivalTime = new Date().getTime() + etaSeconds * 1000;
