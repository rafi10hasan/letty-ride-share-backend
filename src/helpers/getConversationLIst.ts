...
export interface IConversationPopulated {
  ...
  lastSeen: Map<string, Date>; // ✅ Date, not ObjectId
  lastMessage: {
...
export const getConversationList = async (
  ...
  // Extract each conversation's lastSeen Date for the current user
  const lastSeenByConversation = new Map<string, Date | null>();
  for (const conv of conversations) {
    const lastSeenMap = conv.lastSeen as unknown as Record<string, Date>;
    const lastSeenDate: Date | null = lastSeenMap?.[userId] ?? null;
    lastSeenByConversation.set(conv._id.toString(), lastSeenDate);
  }

  const conversationsWithLastSeen = conversations.filter(
    (conv) => lastSeenByConversation.get(conv._id.toString()) != null
  );
  const conversationsWithoutLastSeen = conversations.filter(
    (conv) => lastSeenByConversation.get(conv._id.toString()) == null
  );

  // Run both aggregations in parallel using createdAt Date comparison
  const [withLastSeenAgg, withoutLastSeenAgg] = await Promise.all([
    conversationsWithLastSeen.length > 0
      ? Message.aggregate([
        {
          $match: {
            $or: conversationsWithLastSeen.map((conv) => ({
              conversationId: conv._id,
              senderId: { $ne: userObjectId },
              // createdAt Date comparison instead of _id ObjectId comparison
              createdAt: {
                $gt: lastSeenByConversation.get(conv._id.toString()),
              },
            })),
          },
        },
    ...
    conversationsWithoutLastSeen.length > 0
      ? Message.aggregate([
        {
          $match: {
            conversationId: {
              $in: conversationsWithoutLastSeen.map((c) => c._id),
            },
            senderId: { $ne: userObjectId },
          },
        },
  ...
  io.to(userId).emit(SOCKET_EVENTS.UNREAD_MESSAGE_COUNT, {
