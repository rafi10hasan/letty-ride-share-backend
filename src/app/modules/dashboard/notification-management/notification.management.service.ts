...
const sendSocketNotification = async (payload: TSendNotificationPayload) => {
    ...
    if (audience === "all") {
        await Promise.all([
            ...
            new Promise<void>((resolve) => { io.emit("notification", rest); resolve(); }),
        ]);

    } else if (audience === USER_ROLE.DRIVER) {
        await Promise.all([
            Notification.create({ ...rest, for: USER_ROLE.DRIVER, type: "admin_notification" }),
            new Promise<void>((resolve) => { io.to("driver_channel").emit("notification", rest); resolve(); }),
        ]);

    } else if (audience === USER_ROLE.PASSENGER) {
        await Promise.all([
            Notification.create({ ...rest, for: USER_ROLE.PASSENGER, type: "admin_notification" }),
            new Promise<void>((resolve) => { io.to("passenger_channel").emit("notification", rest); resolve(); }),
        ]);
        ...
        await Promise.all([
            ...
            new Promise<void>((resolve) => { io.to(receiver).emit("notification", rest); resolve(); }),
        ]);
    }
